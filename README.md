...
- `packages/@cosmic/core`: Core React components
- `packages/@cosmic/utils`: Shared React utilities
- `packages/@cosmic/tsconfig`: Shared `tsconfig.json`s used throughout the Turborepo
- `packages/eslint-preset-cosmic`: ESLint preset
...
For `cosmic-core`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `cosmic-core` then instructs the consumer to select the correct format:

```json:cosmic-core/package.json
{
  "name": "@cosmic/core",
  "version": "0.0.0",
  "main": "./dist/index.js",
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `cosmic-core/dist` which contains the compiled output.
...
Each file inside of `cosmic-core/src` is a component inside our design system. For example:

```tsx:cosmic-core/src/Button.tsx
...
- Support using module path aliases like `@cosmic-core` for imports
- Write MDX for component documentation pages

For example, here's the included Story for our `Button` component:

```js:apps/docs/stories/button.stories.mdx
import { Button } from '@cosmic-core/src';
